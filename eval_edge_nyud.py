...
def eval_edge_predictions_forward(save_dir, exp_name):
    ...
    output_file.extend(seism_file[1:18])
        ...
        ## Add method (TODO)
    output_file += ['methods(end+1).name = \'%s\'; methods(end).io_func = @read_one_cont_png; methods(end).legend =     methods(end).name;  methods(end).type = \'contour\';' %(exp_name)]
